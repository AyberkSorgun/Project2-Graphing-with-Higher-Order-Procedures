...
;;;;;;;::;;;;::::;;::::;:::::::;;:;::;;::::::;:;:::;:::;;::;;;:;::;:;;;::;::;;:::;;;
;;;         		 	
;;; asorgun14@ku.edu.tr     Tue Nov  1 11:31:03 2016
;;;         		 	
;;; Before you start:         		 	
;;;         		 	
...
(define (segment-length seg)         		 	
       (sqrt (+(square (-(x-of (end-seg seg)) (x-of (start-seg seg))))(square (-(y-of (end-seg seg)) (y-of (start-seg seg))))))	 	
)         		 	
...
(segment-length seg)
(define seg2 (make-seg (make-point 1 3)
		      (make-point 9 1)))
   	   	    	
(segment-length seg2)
(define seg3 (make-seg (make-point 4 4)
		      (make-point 8 21)))
...
(define (perimeter lst)         		 	
 (if (null? lst) 0 (+ (segment-length (car lst)) (perimeter (cdr lst))))         		 	
)         		 	
...
(define (create-polygon ptlist)
  (helper-create-polygon ptlist ptlist '() )                
)
(define (helper-create-polygon ptlist lst answerlst)
  (if (= (length lst) 1)
      (append answerlst
              (list(make-seg (car lst)(car ptlist))))
        (helper-create-polygon ptlist (cdr lst)
                               (append answerlst (list (make-seg (car lst) (car (cdr lst)))))))
...
(list (+ (x-of p1) (x-of p2)) (+(y-of p1)(y-of p2)))         		 	
)         		 	
...
(define (add-all-points lst)         		 	
 (if (null? lst) (list 0 0) (add-two-points (car lst) (add-all-points (cdr lst)))))         		 	
...
(define (centroid-alt lst)         		 	
 (define x-cords(+ (x-of (car lst)) (car(add-all-points (cdr lst)))))
 (define y-cords (+ (y-of (car lst)) (cadr(add-all-points (cdr lst)))))

  (list (/ x-cords (length lst)) (/ y-cords (length lst)))
)         		 	
...
; Square function is a number to number procedure. Thus it can not take suare of a function, it can only take the square of numbers         		 	
...
;((number x number) -> number) -> (F x F) -> F))        		 	
...
; It will cause an error. Derivative has the type of procedure->procedure. Since it returns a procedure, square function inside of square-a-procedure can not take
;square of a procedure so we would get an error.        		 	
...
; if n=6 both algorithms return the same thing.        		 	
...
(define (diagonal-line pt len)   	   	    	
  (lambda (x) (make-point (+ (* (cos (/ pi 4)) (* len x)) (x-of pt)) (+ (* (cos (/ pi 4)) (* len x)) (y-of pt))))
)
...
(define (connect-ends curve1 curve2)
  (connect-rigidly curve1
                   ((translate (- (x-of (curve1 1)) (x-of (curve2 0))) (- (y-of (curve1 1)) (y-of (curve2 0))))
                    curve2)))        		 	
...
((connect-ends (diagonal-line p1 (* 1 (sqrt 2)))
               (diagonal-line p2 (* 1 (sqrt 2))))
 1)         	
; Value: (10 10)         		 	
...
; They are almost same. There is small differencies between them.
;((draw-points-on g1 200) unit-circle)
;((draw-points-on g2 200) alternative-unit-circle)
; g2 has more points. Still there is little differents between shapes of circles. Also g2 has more points at the bottom, while it goes up the frequency of points decrease.
;((draw-points-squeezed-to-window g1 200) unit-circle)
;((draw-points-squeezed-to-window g2 200) alternative-unit-circle)
; Now they are full shaped pointy circles. g1 looks better because it has same frequency of points on everywhere. g2 has more frequency at right-bottom side.
...
(define (show-points-gosper window level number-of-points initial-curve)
  ((draw-points-on window number-of-points)
   ((squeeze-rectangular-portion -.5 1.5 -.5 1.5)
   (gosper-curve-2 level initial-curve)))
)
(define (gosper-curve-2 level x)                
   ((repeated gosperize level) x))
...
(define arc         		 	
 (put-in-standard-position unit-circle )         		 	
  )         		 	
         		 	
; test         		 	
(show-points-gosper g1 2 3000 arc)
...
((draw-connected-squeezed-to-window g1 200)
 (param-gosper 50 (lambda (level) (/ pi (* 1.3 level)))))
((draw-connected-squeezed-to-window g2 200)
 (param-gosper 70 (lambda (level) (/ pi (* 1.3 level)))))
...
(define koch
  (lambda (curve)
    (put-in-standard-position
     (connect-ends curve
                   (connect-ends ((rotate-around-origin (- (/ pi 3))) curve)
                                 (connect-ends ((rotate-around-origin (/ pi 3)) curve) curve))
                 )
   )
  )
  
  )
(define (param-koch level)
  (if (= level 0)   	   	    	
        unit-line   	   	    	
       (koch  (param-koch (- level 1))
        )
  ))
; Yes.
(define (param-koch-2 level)
  (param-gosper (* 2 level)
                (lambda (level)
                  (if (even? level)
                      (/ pi 6) (- 0 (/ pi 6))))))
                
;((draw-connected g3 200)

((draw-connected g2 200)  ((translate 0 0.5) (param-koch 100)))        		 	
